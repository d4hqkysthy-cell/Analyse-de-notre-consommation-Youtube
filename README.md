...
## 3. Part des vidéos regardées quand on est abonné et non abonné
...
# SÉCURITÉ : On s'assure que la colonne 'Chaine' existe dans df
...
# Fonction : Suis-je abonné ? 
    def verifier_si_abonne(nom):
        return 'Part de vidéos visionnées provenant des abonnements' if str(nom).lower() in liste_abos else 'Part de vidéos visionées sans être abonnés'
    ...
    print(f"\Analyse terminée : {stats_fidelite.get('Chaînes Abonnées', 0)} vidéos vues via tes abonnements.")
else:
    print("\ Fichier abonnements.csv introuvable.")
...
## 4. Nombres d'heures convertit en nombre de jours de visionnage sur Youtube depuis le début
...
## 6. À quelle heure on regarde le plus souvent Youtube ? 
...
df_2025 = df[df['Date_Heure'].dt.year == 2025]
